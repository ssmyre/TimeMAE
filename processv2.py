...
class Trainer_Regress():
    def __init__(self, args, model, train_loader_all, train_loader, test_loader, verbose=False):
        self.args = args
        ...
        self.train_loader_all = train_loader_all
    ...
    def pretrain(self):
        ...
        for epoch in range(self.num_epoch_pretrain):
            self.model.train()
            tqdm_dataloader = tqdm(self.train_loader_all)
            loss_sum = 0
            loss_mse = 0
            loss_ce = 0
            hits_sum = 0
            ...
            self.writer.add_scalar("Pretrain/TrainigLearningRate", self.optimizer.param_groups[0]['lr'], epoch)
            
            if (self.save_freq is not None) and (epoch % self.save_freq == 0) and \
					(epoch > 0):            
                self.model.eval()
                train_rep, train_label = get_rep_with_label(self.model, self.train_loader)
    ...
    def finetune(self):
        ...
        if self.args.resume:
            if self.args.load_pretrained_model:
                ...
                state_dict = torch.load(self.save_path + '/pretrain_model_checkpoint_030.pkl', map_location=self.device)
                try:
                ...
                self.resume_checkpoint = get_latest_checkpoint_from_folder(self.save_path)
                ...
                print(f"Training epochs: {self.epoch} to {self.args.num_epoch}")
