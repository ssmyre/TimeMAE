...
#         return [rep_mask, rep_mask_prediction], [token_prediction_prob, tokens], visible, mask
...
class TimeMAE(nn.Module):
    def __init__(self, args):
        ...
        self.tokenizer = Tokenizer(d_model, args.vocab_size)
        self.reg = Regressor(d_model, args.attn_heads, 4 * d_model, 1, args.reg_layers)
        # self.regression_head = nn.Linear(d_model, 1) # from 2 to 1 for one neurotransmitter
        self.regression_head = nn.Linear(d_model, args.num_targets) # from 2 to 1 for one neurotransmitter
    ...
    def forward(self, x):
        ...
        if self.linear_proba:
            with torch.no_grad():
                # Used only during pretraining or representation learning
                return torch.mean(x, dim=1)
        else:
            # Final regression head (output: [batch_size, 2])
            return self.regression_head(torch.mean(x, dim=1))
